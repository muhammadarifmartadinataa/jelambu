<?php

return [
    // Navigation
    'nav' => [
        'home' => 'Beranda',
        'regencies' => 'Daftar Kabupaten',
        'destinations' => 'Daftar Wisata',
        'chatbot' => 'Chatbot AI',
        'reviews' => 'Ulasan',
    ],
    
    // Common
    'common' => [
        'search' => 'Cari',
        'filter' => 'Filter',
        'sort' => 'Urutkan',
        'view_all' => 'Lihat Semua',
        'view_details' => 'Lihat Detail',
        'read_more' => 'Baca Selengkapnya',
        'share' => 'Bagikan',
        'save' => 'Simpan',
        'rating' => 'Rating',
        'location' => 'Lokasi',
        'facilities' => 'Fasilitas',
        'description' => 'Deskripsi',
        'gallery' => 'Galeri',
        'video' => 'Video',
        'map' => 'Peta',
        'contact' => 'Kontak',
        'about' => 'Tentang',
        'quick_links' => 'Tautan Cepat',
        'online' => 'Online',
        'offline' => 'Offline',
        'loading' => 'Memuat...',
        'no_results' => 'Tidak ada hasil ditemukan',
        'try_different_keywords' => 'Coba gunakan kata kunci yang berbeda',
        'reset_filters' => 'Reset Filter',
    ],
    
    // Home Page
    'home' => [
        'hero_title' => 'Jelajahi Keindahan',
        'hero_subtitle' => 'Wisata Biru Lampung',
        'hero_description' => 'Temukan destinasi wisata biru terbaik di Provinsi Lampung. Dari pantai eksotis hingga laut yang memukau, nikmati pengalaman tak terlupakan di tanah Sai Bumi Ruwai Jurai ini.',
        'explore_destinations' => 'Jelajahi Wisata',
        'ask_jelambu_ai' => 'Tanya Ajel AI',
        'regencies_cities' => 'Kabupaten & Kota di Lampung',
        'regencies_description' => 'Jelajahi kabupaten dan kota di Provinsi Lampung. Setiap daerah memiliki keunikan dan pesona wisata yang berbeda-beda.',
        'popular_destinations' => 'Wisata Populer',
        'popular_description' => 'Destinasi wisata biru terfavorit yang wajib dikunjungi di Lampung dengan rating tertinggi dan ulasan terbaik.',
        'need_help_planning' => 'Butuh Bantuan Merencanakan Liburan?',
        'jelambu_description' => 'Tanya Ajel AI untuk mendapatkan rekomendasi wisata yang sesuai dengan preferensi Anda. AI assistant kami siap membantu 24/7!',
        'chat_with_jelambu' => 'Chat Ajel AI',
        'available_24_7' => 'Tersedia 24/7',
        'multi_language' => 'Multi Bahasa',
        'what_they_say' => 'Apa Kata Mereka',
        'reviews_description' => 'Dengarkan pengalaman luar biasa dari para wisatawan yang telah menjelajahi keindahan Lampung.',
        'stats' => [
            'regencies' => 'Kabupaten',
            'destinations' => 'Destinasi Wisata',
            'average_rating' => 'Rating Rata-rata',
            'visitors' => 'Pengunjung',
        ],
        'ajelai' => [
            'title' => '<span class="text-secondary-400">AJEL</span> <span class="text-primary-600">AI</span><br>Siapa & Kenapa Dia?',
            'description' => 'Ajel AI adalah <strong>AI assistant</strong> yang dikembangkan untuk membantu Anda <strong>merencanakan perjalanan</strong> yang sempurna.<br><br>Ajel AI siap membantu Anda menemukan informasi tentang destinasi wisata biru di Lampung. Tanyakan tentang lokasi, fasilitas, daya tarik, dan banyak lagi!',
            'button' => 'Tanya Sekarang'
        ],
        'scrape' => [
            'title' => 'Ingin tahu komentar orang lain tentang wisata favoritmu?',
            'description' => 'Temukan insight langsung dari percakapan orang-orang di Twitter/X tentang destinasi wisata di Lampung. Kamu bisa cari berdasarkan nama tempat dan lihat pendapat asli mereka.',
            'placeholder' => 'Pilih kata kunci atau destinasi',
            'button' => 'Scrape Tweet'
        ]
    ],
    
    // Regencies Page
    'regencies' => [
        'title' => 'Kabupaten & Kota di',
        'subtitle' => 'Lampung',
        'description' => 'Jelajahi kabupaten dan kota di Provinsi Lampung. Setiap daerah memiliki keunikan dan pesona wisata yang berbeda-beda.',
        'search_placeholder' => 'Cari kabupaten atau kota...',
        'total_regencies' => 'Total Kabupaten & Kota',
        'tourist_destinations' => 'Destinasi Wisata',
        'showing_results' => 'Menampilkan',
        'results_for' => 'hasil untuk',
        'destinations_count' => 'Destinasi',
        'view_destinations' => 'Lihat Wisata',
        'info' => 'Info',
        'ready_to_explore' => 'Siap Menjelajahi Lampung?',
        'explore_description' => 'Mulai petualangan Anda dengan menjelajahi destinasi wisata biru terbaik di setiap kabupaten dan kota di Lampung.',
        'explore_all_destinations' => 'Jelajahi Semua Wisata',
    ],
    
    // Destinations Page
    'destinations' => [
        'title' => 'Destinasi Wisata Biru',
        'subtitle' => 'Lampung',
        'description' => 'Temukan dan jelajahi destinasi wisata biru terbaik di Lampung. Dari pantai eksotis hingga laut yang memukau.',
        'search_placeholder' => 'Cari destinasi wisata...',
        'all_regencies' => 'Semua Kabupaten',
        'highest_rating' => 'Rating Tertinggi',
        'name_a_z' => 'Nama A-Z',
        'location_a_z' => 'Lokasi A-Z',
        'destinations_found' => 'destinasi ditemukan',
        'no_destinations_found' => 'Tidak ada destinasi ditemukan',
        'change_filters' => 'Coba ubah filter pencarian atau kata kunci Anda.',
        'need_personal_recommendations' => 'Butuh Rekomendasi Personal?',
        'ai_description' => 'Biarkan Ajel AI membantu Anda menemukan destinasi wisata yang perfect sesuai dengan preferensi dan budget Anda.',
        'explore_by_regency' => 'Jelajahi per Kabupaten',
        'details' => 'Detail',
        'ask_ai' => 'Tanya Ajel',
    ],
    
    // Detail Page
    'detail' => [
        'about_destination' => 'Tentang Destinasi',
        'location_map' => 'Lokasi & Peta',
        'destination_video' => 'Video Destinasi',
        'interactive_map' => 'Peta Interaktif',
        'click_open_maps' => 'Klik untuk membuka di Google Maps',
        'open_google_maps' => 'Buka di Google Maps',
        'quick_info' => 'Informasi Cepat',
        'village' => 'Desa & Kecamatan',
        'regency' => 'Kabupaten',
        'coordinates' => 'Koordinat',
        'need_more_info' => 'Butuh Info Lebih?',
        'ai_detail_description' => 'Tanya Ajel AI untuk informasi detail tentang destinasi ini!',
        'chat_now' => 'Chat Sekarang',
        'other_destinations' => 'Destinasi Lainnya',
        'full_address' => 'Alamat Lengkap',
        'video_not_found' => 'Video destinasi belum tersedia'
    ],
    
    // Chatbot Page
    'chatbot' => [
        'title' => 'Ajel AI',
        'subtitle' => 'Asisten Wisata Lampung',
        'description' => 'Asisten wisata AI yang siap membantu Anda menemukan destinasi wisata biru terbaik di Lampung. Tanyakan apa saja tentang wisata dan tips perjalanan!',
        'travel_assistant' => 'Asisten Wisata Jelambu',
        'type_message' => 'Ketik pertanyaan anda di sini...',
        'quick_questions' => 'Pertanyaan Cepat:',
        'ready_to_help' => 'Halo! Saya Ajel, asisten wisata Anda. Saya siap membantu Anda menemukan informasi tentang destinasi wisata biru terbaik di Lampung. Ada yang bisa saya bantu?',
        'what_can_ask' => 'Apa yang Bisa Ditanyakan ke Ajel AI?',
        'ai_description' => 'Ajel dilengkapi dengan pengetahuan tentang wisata biru Lampung dan siap membantu merencanakan perjalanan Anda.',
        'features' => [
            'destinations' => [
                'title' => 'Destinasi Wisata',
                'description' => 'Tanyakan tentang berbagai destinasi wisata biru di Lampung, lokasi, dan daya tariknya.',
            ],
            'travel_tips' => [
                'title' => 'Tips Perjalanan',
                'description' => 'Dapatkan tips dan saran untuk merencanakan perjalanan yang sempurna ke Lampung.',
            ],
            'recommendations' => [
                'title' => 'Rekomendasi',
                'description' => 'Minta rekomendasi wisata biru berdasarkan preferensi dan minat Anda.',
            ],
            'detailed_info' => [
                'title' => 'Informasi Detail',
                'description' => 'Tanyakan informasi detail tentang fasilitas, rating, dan aktivitas di setiap wisata.',
            ],
        ],
        'quick_questions_list' => [
            'destinations' => 'Wisata biru apa saja yang ada di Lampung?',
            'pahawang' => 'Ceritakan tentang Pulau Pahawang',
            'dolphins' => 'Dimana bisa lihat lumba-lumba?',
            'snorkeling' => 'Dimana saya bisa snorkeling di Lampung?',
        ],
    ],

    'reviews' => [
        'title' => 'Tarik Ulasan',
        'description' => 'Cari tahu apa yang orang - orang katakan di Platform Twitter atau X tentang destinasi wisata di Lampung.',
        'placeholder' => 'Pilih kata kunci atau destinasi',
        'button' => 'Tarik',
        'button_loading' => 'Menarik...',
        'tweets_found' => 'tweet telah ditemukan',
        'info' => 'Jika tidak ada hasil, coba sekali lagi',
        'not_found' => [
            'title' => 'Tidak ada tweet ditemukan',
            'description' => 'Coba lakukan penarikan beberapa saat lagi',
        ],
        'summary' => 'Kesimpulan'
    ],
    
    // Footer
    'footer' => [
        'description' => 'Temukan keindahan wisata biru Lampung yang menakjubkan. Dari pantai eksotis hingga laut yang memukau.',
        'made_with_love' => 'Dibuat dengan ❤️ untuk Lampung',
        'all_rights_reserved' => 'Semua hak cipta dilindungi.',
    ],
];

